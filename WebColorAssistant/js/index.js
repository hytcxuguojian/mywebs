$(function(){	
	...
	$("#ColorVal").keyup(function(){
		ColorValInputTextChanged();
	});

	$("#ColorRGB input").keyup(function(){
		ColorRGBValInputTextChanged();
	});
	...
	$(".pro").click(function(e){
		...
		$(this).find(".dian").css("left",relativeX+'px');
		$(this).parent().parent().find("input").val(relativeX);
		syncShow();
	});
});
...
function createContent(){
	var list="";
	var html="";
	for(var obj in COLORTABLE){
		var ct=COLORTABLE[obj];
		html+="<div class='section sec"+obj+"'>";
		html+="<h2>"+ct.title+"</h2>";
		html+="<span class='discription'>"+ct.intro+"</span>";
		list+="<h4>"+ct.title+"</h4>";
		list+="<div>";
		for(var s=0,l=ct.table.length;l>s;s++){
			var r=ct.table[s];
			html+="<div class='subsection' id='"+obj+"_"+s+"'>";
			html+="<h3>"+r.name+"</h3>";
			html+="<span class='discription'>"+r.desc+"</span>";
			html+="<div class='groups'>";
			list+="<a href='#"+obj+"_"+s+"'>"+r.name+"</a>";
			for(var c=0,d=r.colors.length;d>c;c++){
				var v=r.colors[c];
				html+="<div class='group'>";
				html+="<span style='background:"+v[0]+"'>";
				html+="<input type='text' value='"+v[0]+"' />";
				html+="</span><span style='background:"+v[1]+"'>";
				html+="<input type='text' value='"+v[1]+"' />";
				html+="</span><span style='background:"+v[2]+"'>";
				html+="<input type='text' value='"+v[2]+"' />";
				html+="</span></div>"
			}
			html+="</div>";
			html+="</div>";
...
/*********Input元素文本值发生变化的函数********/
function ColorValInputTextChanged(){
	var patt1=new RegExp("^#[0-9a-fA-F]{3}$");
	var patt2=new RegExp("^#[0-9a-fA-F]{6}$");
	var ColorVal=$("#ColorVal").val();
	if(patt1.test(ColorVal))
	{
		$(".showColor").css("background-color",ColorVal);
		ColorVal='#'+ColorVal[1]+ColorVal[1]+ColorVal[2]+ColorVal[2]+ColorVal[3]+ColorVal[3];
		$(".showColorVal").val(ColorVal.toUpperCase());
		$(".showColorRGBVal").val(hex16ColorToRGBColor(ColorVal));
		syncColor(ColorVal);
		syncShow();
	}
	if(patt2.test(ColorVal))
	{
		$(".showColor").css("background-color",ColorVal);
		$(".showColorVal").val(ColorVal.toUpperCase());
		$(".showColorRGBVal").val(hex16ColorToRGBColor(ColorVal));
...
function ColorRGBValInputTextChanged(){
	/***验证0-255的整数***/
	var patt=new RegExp("^[0-9]{1}$|^[1-9][0-9]$|^1[0-9][0-9]$|^2[0-4][0-9]$|^25[0-5]$");
	if (patt.test($(".ColorRVal").val())&&patt.test($(".ColorGVal").val())&&patt.test($(".ColorBVal").val())){
		syncShow();
	}
}

//将16进制色转化为RGB色表示方法
function hex16ColorToRGBColor(num){
	var red=parseInt(num[1]+num[2],16);
	var green=parseInt(num[3]+num[4],16);
	var blue=parseInt(num[5]+num[6],16);
	var rgbColor='rgb('+red+','+green+','+blue+')';
	return rgbColor;
}

//将10进制转换为16进制
function hex10Tohex16(num){
	var n=parseInt(num);
	var mod=parseInt(n/16);
	var y=parseInt(n%16);
...
//将RGB色转化为16进制色表示方法
function RGBColorTohex16Color(r,g,b){
	return '#'+hex10Tohex16(r)+hex10Tohex16(g)+hex10Tohex16(b);
}
...
//同步显示
function syncShow(){
	var red=$(".ColorRVal").val();
	var green=$(".ColorGVal").val();
	var blue=$(".ColorBVal").val();
	$(".ColorRVal").val(red);
	$("#dianR").css("left",red+'px');
	$(".ColorGVal").val(green);
	$("#dianG").css("left",green+'px');
	$(".ColorBVal").val(blue);
	$("#dianB").css("left",blue+'px');
	var ColorRGBVal="rgb("+red+","+green+","+blue+")";
	$(".showColor").css("background-color",ColorRGBVal);
	$(".showColorVal").val(RGBColorTohex16Color(red,green,blue));
	$(".showColorRGBVal").val(ColorRGBVal);
}